...
export const AdModal: React.FC<AdModalProps> = ({ isOpen, onAdComplete, onClose }) => {
  ...
  const startRewardedAd = async () => {
    ...
    try {
      ...
      // IMPORTANT: call Monetag directly from this real user click.
      // Calling it from a React useEffect can lose the browser/Telegram
      // user-gesture context required by some ad flows.
          ...
          <div className="p-6 text-center">
            ...
            {status === 'ready' && (
              <>
                ...
                <button
                  ...
                  className="mt-5 w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-black transition-colors"
                >
                  Watch Ad & Continue
                </button>
              </>
            ...
            {status === 'error' && (
              <>
                ...
                <p className="mt-2 text-sm text-slate-400">
                  Monetag did not return an ad. You can try again.
                </p>
                <button
                  onClick={startRewardedAd}
                  className="mt-5 w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-black transition-colors"
                ...
                {onClose && (
                  <button onClick={onClose} className="mt-3 w-full py-3 rounded-xl bg-slate-800 font-bold">
                    Close
                  </button>
                )}
              </>
